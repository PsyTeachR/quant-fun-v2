...
```{r plot2, echo=FALSE, fig.cap="Violin-boxplot of the evaluator data"}
ggplot(ratings2, aes(x = condition, y = Rating)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(aes(fill = condition), width = .2, show.legend = FALSE) + 
  stat_summary(geom = "pointrange", fun.data = "mean_cl_normal")
...
```{r qqplot3, echo=FALSE, fig.cap="Residual plots of listened (left) and read (right) conditions"}
...
a <- qqPlot(listened_residuals$group_resid)
...
b <- qqPlot(read_residuals$group_resid)
...
* In a new code chunk, run both lines of code below and look at their output.Run the below code. According to the Shapiro-Wilk test, is the data normally distributed? `r mcq(c(answer = "Yes", "No"))`
