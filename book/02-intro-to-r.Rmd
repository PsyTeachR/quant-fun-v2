...
Going forward throughout this book, if you're using the server, you'll do an extra step where you also upload them to the server. We're not going to use any data files in this session but let's try an example to make it clear how you get the data files on to the server.
...
library(tidyverse)
funclist <- read_csv("function_names.csv")
seen <- tibble(name = "rnorm()")
sumtable <- inner_join(funclist, seen, "name")

sumtable %>% select(name, package, action, arguments, example) %>%
  knitr::kable()
```
