...
As we have said previously, one of the key aspects in a researcher's toolbox is the knowledge and skill to work with data - regardless of how it comes to you. In fact, when you run an experiment you might get lots of different `r glossary("data type", display = "types of data")` in various different files. For instance, it is not uncommon for an experimental software to create a new file for every `r glossary::glossary("participant", def = "the word used to describe someone who has taken part in a study. Note that subject is outdated and no longer used.", display = "participants", link = FALSE, add_to_table = TRUE)` you run and for each participant's file to contain numerous columns and rows of different data types, only some of which are important. Being able to `r glossary("data wrangling", display = "wrangle")` that data, manipulate it into different layouts, extract the parts you need, and summarise it, is one of the most important skills we will help you learn through this book.
